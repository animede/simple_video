            ...
            <div class="simple-video-section-title with-actions">
                ...
                <div class="simple-video-sequence-flags" aria-label="生成シーケンスオプション">
                    ...
                    <label class="simple-video-sequence-flag" title="（現状は音声対応ワークフローが必要です）">
                        ...
                        <span id="simpleVideoOptAudioLabel">音声も生成</span>
                    </label>
                </div>
            </div>
...
function selectPreset(presetId) {
    ...
    if (audio) {
        const effectiveSteps = getEffectivePresetStepsForCurrentOptions(preset);
        const usesLTX23 = effectiveSteps.some(s => hasNativeAudioWorkflow(s?.workflow));
        const audioLabel = document.getElementById('simpleVideoOptAudioLabel');

        if (usesLTX23) {
            // LTX 2.3 generates audio natively — show toggle to let user choose
            ...
            audio.closest('.simple-video-sequence-flag').title = 'LTX 2.3のネイティブ音声を動画に含めます';
            if (audioLabel) audioLabel.textContent = 'LTX音声を含める';
        } else if (!SIMPLE_VIDEO_STANDALONE_CONFIG.showGenerateAudioOption) {
            ...
            audio.disabled = false;
            audio.closest('.simple-video-sequence-flag')?.classList.remove('disabled');
            audio.closest('.simple-video-sequence-flag').title = '（現状は音声対応ワークフローが必要です）';
            if (audioLabel) audioLabel.textContent = '音声も生成';
        }
    }
...
/**
 * Merge M2V audio with the specified video using video_audio_merge utility.
 * Called when user clicks the "🎵 音楽を追加" button on a concat result.
 */
async function mergeM2VAudioWithCurrentVideo(videoFilename) {
    const state = SimpleVideoUI.state;
    const audioSource = pickPreferredM2VAudioSource(state);
    if (!audioSource?.filename) {
        if (typeof showToast === 'function') showToast('M2V音源がありません', 'error');
        return;
    }
    const mergeVideo = toSimpleVideoBasename(videoFilename);
    const mergeAudio = toSimpleVideoBasename(audioSource.filename);
    if (!mergeVideo || !mergeAudio) {
        if (typeof showToast === 'function') showToast('video/audio ファイル名が不正です', 'error');
        return;
    }
    if (!isAudioFilename(mergeAudio)) {
        if (typeof showToast === 'function') showToast(`音源が音声ファイルではありません: ${mergeAudio}`, 'error');
        return;
    }
    try {
        state.isImageGenerating = true;
        ...
        renderSimpleVideoOutputMedia({
            ...
            title: '音楽追加結果（video_audio_merge）',
            preferMedia: 'video',
        });
        setSimpleVideoProgress('✅ 音楽追加完了', 1);
        if (typeof showToast === 'function') showToast('✅ 音楽を追加しました', 'success');
    } catch (err) {
        const msg = String(err?.message || err || '音楽追加に失敗しました');
        console.error('[SimpleVideo] mergeM2VAudioWithCurrentVideo error:', err);
        setSimpleVideoProgress(`エラー: ${msg}`, 0);
        if (typeof showToast === 'function') showToast(`音楽追加エラー: ${msg}`, 'error');
    } finally {
        state.isImageGenerating = false;
        state.activeJobId = null;
        saveSimpleVideoState();
...
function renderSimpleVideoOutputMedia({ jobId, outputs, title, preferMedia = 'video', showMusicMergeButton = false }) {
    const out = document.getElementById('simpleVideoOutput');
    if (!out) return;
    ...
    if (videoUrl) {
        ...
        const musicMergeAvailable = showMusicMergeButton && !!pickPreferredM2VAudioSource(SimpleVideoUI.state);
        const musicBtnHtml = musicMergeAvailable
            ? '<button class="simple-video-btn" id="simpleVideoMusicMergeBtn">🎵 音楽を追加</button>'
            : '';
        out.innerHTML = `
            <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;width:100%;height:100%;padding:12px;gap:10px;">
                ${heading}
        ...
        `;
        if (musicMergeAvailable) {
            const mergeBtn = document.getElementById('simpleVideoMusicMergeBtn');
            if (mergeBtn) {
                const fn = String(videoOut.filename || '');
                mergeBtn.addEventListener('click', () => mergeM2VAudioWithCurrentVideo(fn));
            }
        }
        return;
    }
...
async function runSceneVideosConcatFromState({ presetId, title = '結合結果（再生成）' } = {}) {
    ...
    const outputs = Array.isArray(outputsPayload?.outputs) ? outputsPayload.outputs : [];
    renderSimpleVideoOutputMedia({ jobId: concatJobId, outputs, title: `${title}（${videos.length}本）`, showMusicMergeButton: true });
}
...
async function startGeneration() {
    ...
    // Note: LTX workflows (including 2.3) strip audio unless generateAudio toggle is ON.
    // The "LTX音声を含める" toggle is shown in the UI when LTX 2.3 workflows are active.
    ...
    try {
        ...
        for (let sceneIndex = sceneStartIndex; sceneIndex < scenePrompts.length; sceneIndex++) {
            ...
            for (let stepIndex = 0; stepIndex < stepsPerScene; stepIndex++) {
                ...
                // LTX: strip audio unless generateAudio is enabled.
                if (String(wf || '').startsWith('ltx2_')) {
                    params.strip_audio = !state.generateAudio;
                    console.log('[SimpleVideo] LTX audio setting: generateAudio=', state.generateAudio, 'strip_audio=', params.strip_audio);
                }
