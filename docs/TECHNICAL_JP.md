...
5. I2I パイプライン変更時は `prependSingleSceneGuard()` の適用漏れがないか確認（分割/パネル出力を防ぐガード）
6. 新規 `mixedTransitions: true` プリセット追加時は `startGeneration()` の `char_edit_i2v_mixed || ext_i2i_i2v_mixed` 分岐条件と `renderTransitionEditor()` 呼び出しも拡張すること
...
#### char_edit_i2v_mixed（キャラ EDIT + I2I + 混在トランジション）

```
@Character + refs ──→ [EDIT] ──→ 合成画像
                                 ↓
                      [I2I × N+1 シーン] ──→ シーン画像群
                                                ↓
                      [境界ごとに FLF or I2V] ──→ クリップ群 ──→ concat ──→ 動画
```

- `mixedTransitions: true` フラグでプロンプト生成時に N+1 シーン分を要求
- `renderTransitionEditor()` がアコーディオン UI で各境界の遷移タイプを表示・編集
- 遷移タイプ: `flf` / `cut` / `crossfade` / `fade_black` / `none`
- I2V 部分は WAN 固定（LTX 非対応）

#### ext_i2i_i2v_mixed（キー画像参照 I2I + 混在トランジション）

```
キー画像 ──→ [I2I リファイン × N+1 シーン] ──→ シーン画像群
             ↑ qwen_i2i_2512_lightning4 (CFG=7.0 / denoise=0.75)
              ↓
             [境界ごとに FLF or I2V] ──→ クリップ群 ──→ concat ──→ 動画
```

- キャラ不要（`requiresCharacter: false`）
- `prependSingleSceneGuard()` で分割/パネル構図を抑制
- その他は `char_edit_i2v_mixed` と同一パイプライン分岐を共有
...
startGeneration()                           // L12225
  ...
  │     └→ prependSingleSceneGuard()           // 分割/パネル出力抑制ガード
  │
  ...
  │     └→ [Wan2.2 FLF 生成]
  │
  ├→ [混在トランジション分岐] (mixedTransitions: true 時)
  │     ├→ renderTransitionEditor()            // アコーディオン UI 表示
  │     └→ [境界ごとに FLF or I2V を動的ディスパッチ]
  │
